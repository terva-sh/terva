...
| `suggest.next_step` | _(no params)_ → `{line}` | one ephemeral completion on the session's own prefix, offering a single line the user might type next while they sit idle. **Records nothing** — not the ask, not the answer, in memory or on disk — and the answer reaches the calling client alone, so unlike `shell.result` it arms nothing for a later turn. Reasoning is turned off explicitly, which is what makes the output cap safe rather than a cap the model spends thinking. An empty `line` is an ordinary answer, not a failure. Optional (a `NextStepController`) |
