...
| `turn.resume` | `{epoch}` | run the agent loop against the transcript as it stands, for a session whose last turn died without producing a reply. It appends nothing (unlike `prompt`) and discards nothing (unlike `turn.retry`): the work is already recorded and only the next request is missing. The daemon routes on the transcript's shape rather than making the client name it — a trailing prompt or a trailing tool-results message runs an ordinary turn on any provider, while a reply the provider cut short (`WireMessage.incomplete`) is extended as a prefill and so needs a provider that can continue one. On the base service for that reason, not behind a controller. Epoch-guarded; starts a turn (`busy` if one runs); bad request when the session is not stuck, or when the cut-short shape meets a provider that cannot continue a prefill |
...
| `tools.display` | → `{tools}` | the rendering hints the session's extensions declared for their own tools (`register_tool display`), keyed by tool name. Each is `{subject?, body?, redact?}`: a template over the call's top-level argument keys written as `{key}`, a renderer name from the closed set `text`/`json`/`diff`/`table`, and argument keys the card masks. **Data, never code** — a client substitutes the template as text and picks a renderer it already has, so an extension can never supply rendering code that runs in the browser. Only tools that declared a hint appear, so `{}` is the usual answer and means every card renders generically. Fetched once per connect rather than carried per call: the hint belongs to the tool, not to the call, and a fix to one applies to cards already on screen. A replay serves nothing here (no extensions run). Read-only, presentation-only, and safely ignored by any client that does not know it |
...
| Method | Params | Effect |
|---|---|---|
| `models.list` | → `{models, reasoning_ladders?, providers?}` | models the workspace can switch to. `providers` is every provider this machine can **reach**, in picker order, which is the set `models.add` accepts. It is a superset of the providers named in `models`, and that gap is the only reason it crosses: a provider the user is logged into that has no models yet contributes no row, so a client deriving the list from `models` would drop exactly the provider an add form is most needed for. Deriving it from `auth.providers` is wrong for the opposite reason, since that view is built from the credential store and omits every keyless backend (ollama, the shared `openai-compatible` slot, named endpoints) |
...
| `models.add` | `{provider, model, values}` | create a `models.json` entry for a model no lower layer knows about, so a user can reach one the catalog has not shipped yet. Same whole-form rule as `models.params.set`, and the same registry does the parsing, so a refusal names the setting rather than the request. **Separate from `models.params.set` because the guards are inverses**: that method requires the id to resolve, and this one requires that it does not. A create flag on the one method would have to switch off the check that stops an edit landing on the wrong provider's copy of a shared id, which is the check worth keeping. An id that already answers is refused as `bad_request` and the caller is told to edit it instead, rather than `conflict`. Nothing moved under them, they asked for the wrong verb. `provider` must be one the user can actually **reach**, asked of the same `build.LoggedInProviders` predicate both pickers filter on: an entry under a provider with no credential writes correctly and then never appears, which reads as a failed save with nothing to explain it. That refusal is `no_credential`, not `bad_request`, because the remedy is a login and a host with a login flow keys on that code to offer one. There is no `clone_from`, though the interface driving this is a clone: the client seeds its form from another model's `models.params` and sends the resulting values, so naming the source would be a second account the daemon could only disagree with. A client seeds from each spec's `default`, **not** its `value`, because `value` is the `models.json` pin and is empty for any source without an entry, so seeding from it would create a model with no context window |
