...
- `ticket_list` / `ticket_search` / `ticket_get` / `ticket_ready` / `ticket_check`: read the repository's `.tickets/` work ledger (the [git-ticket](https://github.com/terva-sh/git-ticket) store `terva ticket` operates on) as structured JSON — filtered listings, full-text search, one ticket in full with its revision and the statuses it may move to next, the claimable set, and the store's check verdict. Present only when a store governs the session cwd; listings page (default 50, cap 200). All five are read-only and survive plan mode. See [standard-tools.md](standard-tools.md#current-standard-bundle).
