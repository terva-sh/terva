...
| `--portable`, `--portable=strict` | Strip terva's harness-local self-context from the system prompt, so the agent runs on what its briefing gave it rather than on descriptions of terva's own machinery. For driving terva as a worker from another harness — the `terva:portable` worker backend spawns `terva rpc --portable`. **It is not a run mode**: it changes prompt content only, leaves the tool surface untouched, and pairs with `-p` / `--json` / `rpc`, which are the headless modes. `--portable` drops the harness-local segments (docs and status hints, auto-swarm, cast, tasks, swarm-child, restricted-workspace, footer) while keeping terva's own discovery; `--portable=strict` drops that discovery too — `AGENTS.md`, the skills manifest, lore — which takes the system prompt to roughly 800 bytes, so use it to prove a briefing stands alone rather than as a way to trim context. |
...
| `--no-memory` | Disable agent-curated durable memory for this run, entirely: no `memory` tool, no injected block, and no memory file read or written. Memory is on by default (config `memory`). |
| `--no-ticket` / `--no-tickets` | Drop the `ticket_*` tools and their prompt guidance for this run. `terva ticket` still reaches the `.tickets/` store, so an agent that opts out works the ledger on the command line instead. The tools are on by default wherever a store governs the cwd (config `tickets`; a project's `.terva/config.json` may also set `false`, never `true`). |
...
- `code_execution_mutating`: the same engine with `write`/`edit` added, for a task that makes many small changes derived from what the program reads. A separate tool rather than a flag, so it classifies as mutating: it prompts like any other write and never appears in `plan` mode. No `bash`. Before running, it reports the calls the script will make, and refuses a script it cannot account for. Same build tag. See [scripting.md](scripting.md).
...
- `ticket_list` / `ticket_search` / `ticket_get` / `ticket_ready` / `ticket_check`: read the repository's `.tickets/` work ledger (the [git-ticket](https://github.com/terva-sh/git-ticket) store `terva ticket` operates on) as structured JSON — filtered listings, full-text search, one ticket in full with its revision and the statuses it may move to next, the claimable set, and the store's check verdict. Present only when a store governs the session cwd; listings page (default 50, cap 200). All five are read-only and survive plan mode. `ticket_get` also resolves the store half of any `ticket:<store>/<id>` reference it returns: a store the `ticket_stores` registry names comes back with its path in `store_path`, and one it does not name comes back with a `store_note` that names the missing store and lists the stores the session does have. The lookup stops at the name — no read tool opens the other store, and `ticket_store` remains the only way to change which store the tools work against. See [standard-tools.md](standard-tools.md#current-standard-bundle).
- `ticket_create` / `ticket_update` / `ticket_transition` / `ticket_claim` / `ticket_comment` / `ticket_fix`: mutate that same ledger — file a draft, change fields atomically, move status, claim or release work, append a comment or note, and repair the store. Workspace-mutation class: pruned in plan mode, refused by a headless non-yolo gate. `ticket_fix` is in this half despite the name it shares with `ticket_check`: it moves and rewrites files, and `dry_run: true` is how you read the repairs before it makes them. Every mutation except create and fix requires `if_revision` from `ticket_get`, and a stale value refuses with the current revision named. Writes record terva's own actor identity (`agent:terva/<persona>`); nothing publishes — changes land as files in the store for the user's next commit. `ticket_claim` also seeds the session task list, one task per acceptance criterion that is not checked yet, each carrying the ticket id and criterion index it came from; pass `seed_tasks: false` to claim without touching the list. Closing one of those tasks with evidence checks its criterion, and a move to `done`, `archived`, or `blocked` appends that ticket's share of the session worklog as a note. See [standard-tools.md](standard-tools.md#current-standard-bundle).
- `ticket_store`: select which store the other `ticket_*` tools work against. Present only where the user config key `ticket_stores` names at least one store that opens, so a user who configured none pays no schema cost. That key maps a short name to a path, it lives on the user layer alone, and a project's `.terva/config.json` cannot name a path at all. A store name is lower case and matches `[a-z0-9-]+`, because the same name is the `<store>` half of a `ticket:<store>/<id>` reference (see `.tickets/CONVENTIONS.md`); `git ticket refs` compares the identifier exactly, so a name in mixed case would split a qualified ref into two that no lookup joins. The same registry backs the qualified-ref lookup described above, and a session reads it whether or not this tool registers: a session that configured no store is precisely the one that needs to say which store a ref names and it does not have. Call the tool with no argument to read the configured stores and the active one, give `store` as a configured name to switch, and give `store` as `workspace` to return to the store of this repository. The selection holds for the rest of the session, every other ticket tool follows it, and the ticket context card names a store once the session moves off the workspace one. A path may name the directory that holds `.tickets` or the `.tickets` directory itself. Nothing here creates a store: run `git ticket init` in the directory you want one in. Read-only class, like `activate_tools`, because it changes which store the tools reach and grants no authority of its own; each write to the selected store still faces its own gate. `/status` lists the configured stores and any entry it refused. See [permissions.md](permissions.md#ticket-stores-outside-the-workspace).
- `ticket_init`: create the `.tickets/` store and write git-ticket's agent workflow block to `AGENTS.md`, the same work as `terva ticket init --instructions`. Present only on the *inverse* of the gate above, where no store governs the session cwd, so no session ever carries this tool and the store-conditional ones together. Workspace-mutation class, and pruned in plan mode: it writes `.tickets/`, `.gitattributes`, and `AGENTS.md` into the repository. On success it asks the host to re-resolve the tool set, so the others arrive on the model's next step instead of the next session; hosts that can do that are the TUI/web daemon, rpc, and ACP, and the result says which case this session is. A new store records a first actor, and git-ticket signs any later flag-less write with it, so the tool asks the user who that should be (suggesting a `human:` id built from `git config user.name`) and remembers the answer in the user config key `ticket_actor` for the next repository on the machine. `terva ticket init` shares that resolution, so an answer given to either one serves both. `--no-ticket` and `tickets: false` drop it with the rest. See [standard-tools.md](standard-tools.md#current-standard-bundle).
...
| `terva ticket ...` | Operate the repository's `.tickets/` work ledger. This embeds [git-ticket](https://github.com/terva-sh/git-ticket)'s whole command surface, so `terva ticket list` runs the same code as `git ticket list` **at the version terva pins** (`go.mod`, currently v0.14.3), and the exit statuses are git-ticket's own. A separately installed `git-ticket` on your `PATH` can be a different version, and then the two surfaces differ. One verb is terva's own and not git-ticket's: `terva ticket groom` reads the draft pool and reports which drafts are worth a person's attention, with reasons, and what is not ready with why. It is one weak-tier model call (the `swarm_tiers` ladder, falling back to the host model with a warning that names the cost), it never changes a status, and by default it writes nothing at all, which is what makes it safe to run from a systemd timer. `--note` records the report as a note on each candidate and on nothing else. A store with no drafts is answered before any model is resolved, so it needs no credential. The design is tier 5 of the `ticket-agent-workflow` proposal, which the development tree carries under `docs/proposals/`. `terva ticket init` starts a store in the current repository; add `--instructions` to also write the agent workflow block into `AGENTS.md`. A new store records a first actor, and git-ticket signs every later flag-less write with it, so `init` resolves one the same way the `ticket_init` tool does: an explicit `--actor` wins, then the answer you gave for an earlier store (user config key `ticket_actor`), then a question when stdin is a terminal. When you did not type `--actor`, the command names the actor it recorded. A run with no terminal and nothing remembered creates the store, never blocks on the question, and warns that the roster is empty. The store resolves from the current directory, with git-ticket's `--store PATH` for unusual layouts. Unaffected by `--no-ticket` and by `tickets: false`, which drop the model's tools and never the ledger. `terva ticket ui` opens the ledger as a full-screen terminal browser rather than printing: it needs a terminal on stdin and stdout, and refuses with `the TUI needs a terminal on stdin and stdout` when piped. The interactive TUI reaches the same UI with `/ticket` (see [tui.md](tui.md)). Both refuse a store whose `actors:` roster is empty, naming the lines to add, because git-ticket rejects every write to such a store and an editor is the only repair today. `terva ticket help` lists every command. |
...
| `terva ctl VERB [PARAMS_JSON]` | Send one control-plane (ctrlproto) verb to a running terva and print the reply — the same wire the TUI, `terva attach`, and the web client speak, for the verbs no surface has a button for yet. `--list` names every verb this build serves; `--select a,b.c` prints only those dotted paths — paths crossing the same array come back as **rows**, one object per element, so a field missing from some elements is a key that row lacks rather than a silently shorter array; `--shape` prints the reply's structure and byte sizes and **never its values**, which is the right first look at any list verb whose reply you have not seen. `--sess ID` addresses a session (`#workspace` for the workspace itself). A debugging and automation tool: it sends whatever verb you name, including destructive ones. |
