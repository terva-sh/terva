...
| `terva ticket ...` | Operate the repository's `.tickets/` work ledger. This embeds [git-ticket](https://github.com/terva-sh/git-ticket)'s whole command surface, so `terva ticket list` runs the same code as `git ticket list` **at the version terva pins** (`go.mod`, currently v0.14.1), and the exit statuses are git-ticket's own. A separately installed `git-ticket` on your `PATH` can be a different version, and then the two surfaces differ. `terva ticket init` starts a store in the current repository; add `--instructions` to also write the agent workflow block into `AGENTS.md`. The store resolves from the current directory, with git-ticket's `--store PATH` for unusual layouts. Unaffected by `--no-ticket` and by `tickets: false`, which drop the model's tools and never the ledger. `terva ticket help` lists every command. |
