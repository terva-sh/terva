...
| `terva ticket ...` | Operate the repository's `.tickets/` work ledger. This embeds [git-ticket](https://github.com/terva-sh/git-ticket)'s whole command surface, so `terva ticket list` and `git ticket list` are the same code reached two ways, and the exit statuses are git-ticket's own. The store resolves from the current directory, with git-ticket's `--store PATH` for unusual layouts. Unaffected by `--no-ticket` and by `tickets: false`, which drop the model's tools and never the ledger. `terva ticket help` lists every command. |
