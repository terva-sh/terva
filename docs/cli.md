...
- `ticket_list` / `ticket_search` / `ticket_get` / `ticket_ready` / `ticket_check`: read the repository's `.tickets/` work ledger (the [git-ticket](https://github.com/terva-sh/git-ticket) store `terva ticket` operates on) as structured JSON — filtered listings, full-text search, one ticket in full with its revision, the claimable set, and the store's check verdict. Present only when a store governs the session cwd; listings page (default 50, cap 200). All five are read-only and survive plan mode. See [standard-tools.md](standard-tools.md#current-standard-bundle).
- `ticket_create` / `ticket_update` / `ticket_transition` / `ticket_claim` / `ticket_comment`: mutate that same ledger — file a draft, change fields atomically, move status, claim or release work, append a comment or note. Workspace-mutation class: pruned in plan mode, refused by a headless non-yolo gate. Every mutation except create requires `if_revision` from `ticket_get`, and a stale value refuses with the current revision named. Writes record terva's own actor identity (`agent:terva/<persona>`); nothing publishes — changes land as files in the store for the user's next commit. See [standard-tools.md](standard-tools.md#current-standard-bundle).
