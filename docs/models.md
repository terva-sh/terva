...
The `openai-codex` and `anthropic` paths implement this today; other providers ignore the setting. Note that OpenAI-compatible chat providers that emit `reasoning_content` (DeepSeek, Kimi, and similar) already persist readable reasoning unconditionally — this setting is not what governs them.
