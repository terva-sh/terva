...
func (s *serveState) handle(ctx context.Context, f Frame) {
	...
	case MethodCardsList, MethodCardsGet, MethodCardsImport, MethodCardsEdit, MethodCardsDelete, MethodCardsExport, MethodCardsLint, MethodCardsFavorite, MethodCardsHistory, MethodCardsRestore, MethodCardsRevision:
		...
		case MethodCardsHistory:
			var p CardHistoryParams
			if err := f.Bind(&p); err != nil {
				s.badReq(f.ID, err)
				return
			}
			v, err := cc.CardsHistory(ctx, p)
			s.respond(f.ID, v, err)
		case MethodCardsRestore:
			var p CardRestoreParams
			if err := f.Bind(&p); err != nil {
				s.badReq(f.ID, err)
				return
			}
			v, err := cc.CardsRestore(ctx, p)
			s.respond(f.ID, v, err)
		case MethodCardsRevision:
			var p CardRevisionParams
			if err := f.Bind(&p); err != nil {
				s.badReq(f.ID, err)
				return
			}
			v, err := cc.CardsRevision(ctx, p)
			s.respond(f.ID, v, err)
		default:
