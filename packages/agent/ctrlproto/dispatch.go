...
var (
	...
	noSessionState = plain("session state is not available here")
	noNote         = plain("the author's note is not available here")
	noShellResult  = plain("shell results are not available here")
	...
	noNextStep     = plain("next-step suggestions are not available here")
