...
func (s *wsSession) permissionsAction(action string, args map[string]string) error {
	...
	case "remove_rule":
		rule := config.PermissionRuleConfig{Tool: args["tool"], Args: args["args"], Decision: strings.ToLower(args["decision"])}
		var err error
		if args["scope"] == "project" {
...
func removeUserPermissionRule(match config.PermissionRuleConfig) error {
	return config.MutateConfig(func(c *config.Config) {
