...
type Config struct {
	...
	// or "off" (never automatic; context-limit errors surface and the
	// user compacts by hand with /compact). Read live per check, so an
	// edit applies without restarting the session. Unknown values fall
	// back to "steps".
...
// raati-deliberation.md). User layer ONLY, like SwarmTiers — a cloned
// project's config must be able to neither enable deliberation spend
// nor redirect which models sit on the panel.
type RaatiConfig struct {
	// ConveneTool offers the raati_convene tool to interactive agents.
	...
	//     still works, and no model owns a prior across deliberations.
	//   "fixed" — pool order (or SeatMap): stable panel character,
	//     comparable across runs, but each model's idiosyncrasies fuse
	//     with its seat's prior.
	//   "turn" — reshuffled per voting round: the seat persists, the
	//     weights behind it rotate mid-deliberation. Strongest defense
	//     against fixed model↔prior bias, at a real cost: the final
	//     round respawns every seat cold (no prompt-cache reuse across
	//     rounds, question + evidence re-read per seat per round).
	SeatOrder string `json:"seat_order,omitempty"`
	// SeatMap, for seat_order "fixed" only, remaps pool indices onto
	// seats by position: seat i draws the pool entry SeatMap[i]. Must
	// be a permutation of 0..seats-1 (e.g. [2,0,1]).
	SeatMap []int `json:"seat_map,omitempty"`
	// Profiles are named convening bundles selected BY NAME at convene
	// time (tool arg / web form): the caller picks which profile, only
	// this config says what a profile means. That split is the point —
	// an agent whose plan is being judged must never seat its own
	// judges, so seat composition stays user-authored while the choice
	// of purpose ("code-review" vs "ethics" vs "triage") stays with
	// the caller. Description is the selection signal: the convene
	// tool enumerates it verbatim so profiles can be chosen by what
	// they are for.
	//
	// terva ships built-ins (triage, counsel, code-review, ethics —
	// raati.BuiltinProfiles); an entry here with the same name
	// replaces the built-in wholesale.
	Profiles map[string]RaatiProfileConfig `json:"profiles,omitempty"`
	// BuiltinProfiles, when explicitly false, hides the shipped
	// profile set entirely. Unset means on.
	BuiltinProfiles *bool `json:"builtin_profiles,omitempty"`
}

// RaatiBindingConfig is one seat's exact pin in RaatiConfig.Level2.
type RaatiBindingConfig struct {
	Provider string `json:"provider"`
	Model    string `json:"model"`
}

// RaatiProfileConfig is one named convening bundle in
// RaatiConfig.Profiles. Every field is a DEFAULT: a value the convene
// call itself supplies wins over the profile — except seat composition
// (Seats), which no call may supply. Fields are validated when the
// profile is used, not when the config loads, so a typo errors the
// convening with a message instead of silently vanishing the profile.
type RaatiProfileConfig struct {
	// Description says what the profile is for; shown wherever
	// profiles are picked (the tool description, the convene form).
	Description string `json:"description,omitempty"`
	// Level is the rigor the profile convenes at: 0-2, "auto" (the
	// highest level this config supports at convene time), or
	// "auto:N" (auto, capped at N — collapse a profile to one
	// provider's ladder even when cross-provider seats exist). Unset
	// leaves the level to the call. An explicit 0 pins the cheap
	// correlated panel on purpose.
	Level *RaatiProfileLevel `json:"level,omitempty"`
	// Seats pins the panel explicitly (implies level 2), one binding
	// per seat like Level2 — a per-profile pool that wins over
	// raati.level2 when this profile convenes.
	Seats []RaatiBindingConfig `json:"seats,omitempty"`
	// SeatOrder / SeatMap override the top-level pool→seat policy for
	// this profile (same values as RaatiConfig.SeatOrder / SeatMap).
	SeatOrder string `json:"seat_order,omitempty"`
	SeatMap   []int  `json:"seat_map,omitempty"`
	// Class is the default decision class (advisory | gate | veto).
	Class string `json:"class,omitempty"`
	// SingleRound / Inquire / Converge default the deliberation shape:
	// blind-only ballots, the inquiry gap ("record" or "convener"),
	// and the flip-gated third round.
	SingleRound *bool  `json:"single_round,omitempty"`
	Inquire     string `json:"inquire,omitempty"`
	Converge    *bool  `json:"converge,omitempty"`
}

// RaatiProfileLevel is RaatiProfileConfig.Level's wire shape: a bare
// number (0-2), the string "auto", or "auto:N" for auto with a
// ceiling. Kept as parsed parts so the profile machinery never
// re-tokenizes the string.
type RaatiProfileLevel struct {
	Value   *int // set for a numeric level
	Auto    bool
	Ceiling int // meaningful when Auto; a bare "auto" means 2
}

func (l *RaatiProfileLevel) UnmarshalJSON(b []byte) error {
	...
	if s[0] != '"' {
		...
		if err := json.Unmarshal(b, &n); err != nil {
			return fmt.Errorf("raati profile level: want 0-2, \"auto\", or \"auto:N\", got %s", s)
		}
		l.Value = &n
		return nil
	}
	...
	switch {
	case raw == "auto":
		l.Auto, l.Ceiling = true, 2
		return nil
	case strings.HasPrefix(raw, "auto:"):
		n, err := strconv.Atoi(strings.TrimPrefix(raw, "auto:"))
		if err != nil || n < 0 || n > 2 {
			return fmt.Errorf("raati profile level: \"auto:N\" wants N in 0-2, got %q", raw)
		}
		l.Auto, l.Ceiling = true, n
		return nil
	}
	// Numeric strings pass through for config-editor friendliness.
	if n, err := strconv.Atoi(raw); err == nil {
		l.Value = &n
		return nil
	}
	return fmt.Errorf("raati profile level: want 0-2, \"auto\", or \"auto:N\", got %q", raw)
}

func (l RaatiProfileLevel) MarshalJSON() ([]byte, error) {
	switch {
	case l.Auto && l.Ceiling != 2:
		return json.Marshal(fmt.Sprintf("auto:%d", l.Ceiling))
	case l.Auto:
