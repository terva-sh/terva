...
// TestRaatiProfileLevelParsing: the level field takes 0-2, "auto", or
// "auto:N" — the ceiling collapses an auto profile to one provider's
// ladder even when cross-provider seats exist.
func TestRaatiProfileLevelParsing(t *testing.T) {
	parse := func(raw string) (RaatiProfileLevel, error) {
		var l RaatiProfileLevel
		err := json.Unmarshal([]byte(raw), &l)
		return l, err
	}
	if l, err := parse(`2`); err != nil || l.Value == nil || *l.Value != 2 || l.Auto {
		t.Errorf("numeric = %+v, %v", l, err)
	}
	if l, err := parse(`"auto"`); err != nil || !l.Auto || l.Ceiling != 2 {
		t.Errorf("auto = %+v, %v", l, err)
	}
	if l, err := parse(`"auto:1"`); err != nil || !l.Auto || l.Ceiling != 1 {
		t.Errorf("auto:1 = %+v, %v", l, err)
	}
	for _, bad := range []string{`"auto:3"`, `"max"`, `true`} {
		if _, err := parse(bad); err == nil {
			t.Errorf("%s parsed without error", bad)
		}
	}
	// Round-trip: a rewritten config must not corrupt the field.
	for _, raw := range []string{`2`, `"auto"`, `"auto:1"`} {
		l, _ := parse(raw)
		out, err := json.Marshal(l)
		if err != nil || string(out) != raw {
			t.Errorf("round-trip %s -> %s (%v)", raw, out, err)
		}
	}
}
...
func TestStatusLineMaxWidth(t *testing.T) {
	for _, tc := range []struct {
		...
		if got := c.StatusLineMaxWidth(); got != tc.want {
			t.Errorf("%s: StatusLineMaxWidth() = %d, want %d", tc.name, got, tc.want)
		}
	}
}
