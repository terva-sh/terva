...
// Ext and ID identify the panel currently on screen: the owning
// extension and the extension-scoped panel id. Callers compare them to
// decide whether an incoming panel event addresses this panel.
func (d *ExtPanelDialog) Ext() string { return d.ext }
func (d *ExtPanelDialog) ID() string  { return d.id }
...
func (d *ExtPanelDialog) Render(th tui.Theme, width int) []string {
	...
	for _, l := range d.lines {
		plain := widgets.StripANSIBytes(l)
		...
		for _, seg := range tui.WrapANSILineKeepStyle(l, limit) {
			out = append(out, styleExtPanelLine(th, seg, widgets.StripANSIBytes(seg), width, selected))
		}
	}
