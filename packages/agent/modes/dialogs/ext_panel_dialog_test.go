...
func TestExtPanelWrapsLongLines(t *testing.T) {
	...
	for _, row := range out {
		if w := runewidth.StringWidth(widgets.StripANSIBytes(row)); w > width {
			t.Errorf("row exceeds width %d (visible=%d): %q", width, w, widgets.StripANSIBytes(row))
		}
	}
	...
	selRows := 0
	for _, row := range out {
		if strings.Contains(widgets.StripANSIBytes(row), "alpha") {
			if !strings.Contains(row, sel) {
				t.Errorf("wrapped selected row missing highlight: %q", widgets.StripANSIBytes(row))
			}
			selRows++
		}
	}
	...
	blank := false
	for _, row := range out {
		if strings.TrimSpace(widgets.StripANSIBytes(row)) == "" {
			blank = true
		}
	}
	if !blank {
