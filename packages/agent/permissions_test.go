...
func TestResolveApprovalModePrecedence(t *testing.T) {
	cases := []struct {
		...
		{"flag beats config", Args{Approval: "yolo"}, Config{Approval: "ask"}, core.ApprovalYolo},
		{"no-yolo beats config", Args{NoYolo: true}, Config{Approval: "yolo"}, core.ApprovalAsk},
		{"bad config value falls back to interactive default", Args{Mode: ModeInteractive}, Config{Approval: "bogus"}, core.ApprovalWorkspace},
	}
	for _, c := range cases {
