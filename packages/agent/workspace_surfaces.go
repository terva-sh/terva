...
func (s *wsSession) surfaceList() []ctrlproto.SurfaceMeta {
	...
	if s.ws != nil && s.ws.mcpAdapter != nil && len(listMCPServers(s.cwd, s.trusted.Load(), s.ws.mcpAdapter.mgr)) > 0 {
...
func (w *Workspace) Catalog(ctx context.Context, lang string) (ctrlproto.CatalogView, error) {
	_ = i18n.Configure(Language(), TervaHome())
	if lang == "" {
		lang = i18n.ActiveLang()
	}
	doc, err := i18n.WebCatalog(lang, TervaHome())
	if err != nil {
