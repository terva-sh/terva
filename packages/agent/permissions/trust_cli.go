...
func resolveTrust(cwd string, forced bool, store config.TrustStore) config.TrustState {
	if forced {
		return config.TrustGranted
	}
	if ok, _ := store.IsTrusted(cwd); ok {
		return config.TrustGranted
	}
...
func ResolveTrustState(cwd string, forced bool) config.TrustState {
	...
	return resolveTrust(cwd, forced, store)
}
...
func WarnRestrictedWorkspace(cwd string, trusted bool) {
	if trusted || !config.HasGatedProjectContent(cwd) {
		return
	}
