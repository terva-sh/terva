...
func TestBuildImageRegistryOptIn(t *testing.T) {
	...
	if reg, _ := buildImageRegistry(Config{}); reg.Len() != 0 {
		t.Errorf("no image block should yield an empty registry, got %d", reg.Len())
	}

	// Explicit disable → off even with a backend configured.
	off := false
	cfg := Config{Image: &ImageConfig{
		Enabled:  &off,
		Backends: map[string]ImageBackendConfig{"a": {BaseURL: "http://x", Model: "m"}},
	}}
...
func TestBuildImageRegistryExplicitBackend(t *testing.T) {
	cfg := Config{Image: &ImageConfig{
		Backend: "openai",
		Backends: map[string]ImageBackendConfig{
...
func TestBuildImageRegistryA1111(t *testing.T) {
	...
	bad := Config{Image: &ImageConfig{Backends: map[string]ImageBackendConfig{"local": {Protocol: "a1111"}}}}
...
func TestBuildImageRegistryComfyUI(t *testing.T) {
	...
	bad := Config{Image: &ImageConfig{Backends: map[string]ImageBackendConfig{
...
func TestBuildImageRegistryUnknownProtocol(t *testing.T) {
	cfg := Config{Image: &ImageConfig{
		Backends: map[string]ImageBackendConfig{"a": {Protocol: "midjourney"}},
	}}
	reg, status := buildImageRegistry(cfg)
	if reg.Len() != 0 {
