...
func TestBundleSkillsDiscovered(t *testing.T) {
	...
	found, errs := skills.Discover(home, testsupport.TempDir(t), "", true, true, skills.Gate{TrustProject: true})
	if len(errs) > 0 {
...
func TestBundleSkillsSkipDisabledExtension(t *testing.T) {
	...
	found, _ := skills.Discover(home, testsupport.TempDir(t), "", true, true, skills.Gate{TrustProject: true})
	for _, s := range found {
