...
test('stage: the Worlds shelf, chat-in-World, and promotion', async ({ page }) => {
  ...
  // The sheet: characters with a way in, and the member chats.
  await worldTile.click()
  await expect(page.locator('.stage-worldsheet')).toBeVisible()
  await expect(page.locator('.stage-worldsheet .stage-cast__member')).toHaveCount(2)
  await expect(page.locator('.stage-worldsheet .stage-yourchats__title')).toHaveText('The Ledger Job')

  // "Chat" as Rook → a session created INSIDE the World.
  await page
    .locator('.stage-worldsheet .stage-cast__member', { hasText: 'Rook' })
    .locator('.stage-worldsheet__chat')
    .click()
