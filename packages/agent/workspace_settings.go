...
func (s *wsSession) settingsAction(action string, args map[string]string) error {
	...
	case "auto_title":
		on := val == "true"
		if err := MutateConfig(func(c *Config) { c.AutoTitle = &on }); err != nil {
			return ctrlproto.Errorf(ctrlproto.CodeInternal, "save config: %v", err)
		}
	case "auto_swarm":
	...
	case "language":
		...
		if err := i18n.Configure(val, TervaHome()); err != nil {
			return ctrlproto.Errorf(ctrlproto.CodeInternal, "configure locale: %v", err)
		}
		if err := MutateConfig(func(c *Config) { c.Language = val }); err != nil {
			return ctrlproto.Errorf(ctrlproto.CodeInternal, "save config: %v", err)
		}
