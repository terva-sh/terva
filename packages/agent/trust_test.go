...
func TestMovedDirNotTrusted(t *testing.T) {
	...
	if err := TrustPath(orig, false); err != nil {
		t.Fatal(err)
	}
...
func TestResolveTrustPrecedence(t *testing.T) {
	...
	cases := []struct {
		...
		{"default untrusted", Args{CWD: repo}, empty, TrustRestricted},
		{"--trust one-shot trusts", Args{CWD: repo, Trust: true}, empty, TrustGranted},
		{"store entry trusts", Args{CWD: repo}, withRepo, TrustGranted},
		{"store entry does not trust other dir", Args{CWD: other}, withRepo, TrustRestricted},
		{"--trust wins even off-store", Args{CWD: other, Trust: true}, withRepo, TrustGranted},
	}
	for _, c := range cases {
