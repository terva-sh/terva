...
func TestSetProjectExtensionDisabled(t *testing.T) {
	...
	if err := config.SetProjectExtensionDisabled(cwd, "foo", true); err != nil {
		t.Fatal(err)
	}
	...
	if err := config.SetProjectExtensionDisabled(cwd, "bar", true); err != nil {
		t.Fatal(err)
	}
	m = readJSON(t, path)
	...
	if err := config.SetProjectExtensionDisabled(cwd, "foo", false); err != nil {
		t.Fatal(err)
	}
	...
	if err := config.SetProjectExtensionDisabled(cwd, "bar", false); err != nil {
		t.Fatal(err)
	}
...
func TestSetManifestEnabled(t *testing.T) {
	...
	if err := config.SetManifestEnabled(dir, false); err != nil {
		t.Fatal(err)
	}
	m := readJSON(t, mf)
	...
	if err := config.SetManifestEnabled(dir, true); err != nil {
		t.Fatal(err)
	}
...
// config.SetProjectModel writes provider/model into .terva/config.json, preserves
...
func TestSetProjectModel(t *testing.T) {
	...
	b, _ := json.MarshalIndent(m, "", "  ")
	os.WriteFile(path, b, 0o644)
	if err := config.SetProjectModel(cwd, "openai", "gpt-5"); err != nil {
		t.Fatal(err)
	}
	m = readJSON(t, path)
	...
	if err := config.SetProjectModel(cwd, "", ""); err != nil {
		t.Fatal(err)
	}
	m = readJSON(t, path)
	if _, ok := m["model"]; ok {
...
func TestAppendUnrootedExtensions(t *testing.T) {
	...
	out := []extensions.Info{{Name: "installed", Scope: "global"}}
	live := []sessionExt{
