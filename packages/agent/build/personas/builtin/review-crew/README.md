...
Every charter ends with the same sub-agent deliverable contract: when
dispatched via swarm, the final message IS the report the coordinator
receives, so it must carry the complete findings — never a status line or
task-tracker housekeeping (the 2026-07-08 self-review lost two specialists'
findings exactly that way). Keep the paragraph when adding a crew member;
`TestReviewCrewChartersCarryDeliverableContract` pins it.
