...
func TestMachinePersonaNotice(t *testing.T) {
	builtin := persona.Persona{Name: "Seppä", Source: "embedded:personas/seppa.md"}
	if got := persona.MachineNotice("seppa", builtin, "card is in good shape"); got != "card is in good shape" {
		t.Errorf("builtin persona must pass the note through unchanged, got %q", got)
	}
	if got := persona.MachineNotice("seppa", builtin, ""); got != "" {
		t.Errorf("builtin persona with no note must stay empty, got %q", got)
	}

	override := persona.Persona{Name: "Seppä", Source: "/home/u/.terva/personas/seppa.md"}
	got := persona.MachineNotice("seppa", override, "card is in good shape")
	...
	if bare := persona.MachineNotice("seppa", override, ""); bare == "" || strings.Contains(bare, "\n") {
